...
function extractKeywords(title, body) {
  ...
  // Get the most distinctive words (4+ chars to skip noise)
...
async function searchCandidateIssues(octokit, owner, repo, issueNumber, issueTitle, issueBody) {
  ...
  // Sort by relevance (prefer issues with more label overlap and recency)
...
async function detectDuplicates(client, issueTitle, issueBody, candidates) {
  ...
  const truncatedBody = issueBody.slice(0, 3000);
...
function buildComment(matches, candidates) {
  ...
  for (const match of matches) {
    ...
    lines.push(`### ${icon} #${match.issue_number} — ${candidate.title} (${status})`);
    lines.push("");
    lines.push(`**Why related:** ${match.explanation}`);
